...
public:
  ...
  int read(FILE *fp);
  const char *get_name() const { return (const char *)name.get(); }
...
enum cond_type { COND_NONE, EQ, NEQ, LT, GT, LE, GE, LIKE };
...
public:
  ...
  void adjust_phone(command &cmd, int new_phone) {
    init(cmd.get_name(), new_phone, cmd.group);
  }
  // prints
...
class parse // input string
{
private:
  char **words = nullptr;
  int perstr = 0; // do not forget to -1
public:
  void destroy();
  void print();
  parse() = default;
